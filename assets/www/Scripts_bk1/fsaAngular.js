...
alert('online msg: ' + window.navigator.onLine);

/*
var connectionStatus = false;

$(document).on('pagebeforeshow', '#FSA', function () {
...
function LoginController($scope, $location, $rootScope, $http, $routeParams) {
    ...
    if (fsaApp.session.isSessionAvailable()) {
      ...
      //      if (window.navigator.onLine != true){
            console.log('local storage: ' + window.localStorage.getItem('userlogin'))
            var logintmp = JSON.parse(window.localStorage.getItem('userlogin'));
           
            var loginArr = JSON.stringify(logintmp);
 //           console.log('loginArr: ' + loginArr);
 //           console.log(offlineloginid = logintmp["User"]);
  //          console.log(offlinekey = logintmp['Key']);
                ...
                .success(function (data, status, headers, config) {               
                    console.log('login data console: ' + JSON.stringify(data));
...
function TasksController($rootScope, $scope, $location, $http, $routeParams) {
    ...
    var taskDetailsString = '';
    var taskNotesString = '';
    var taskCounterString = '';
    var taskSystemDownString = '';
    var taskManagerApprovalString = '';

    var taskDetailsArray = [];
    var taskNotesArray = [];
    var taskCounterArray = [];
    var taskSystemDownArray = [];
    var taskManagerApprovalArray = [];
    ...
    .success(function (data, status, headers, config) {
        ...
        var taskscount = data.length;
        var incidentID;

        console.log('calling create local storage: ' + data.length);       
        //       console.log(data);

        window.localStorage.setItem('tasks', JSON.stringify(data));
  
        //        var taskDetailsArray = new Array(); //[];
        //        var taskNotesArray = new Array(); //[];
        //        var taskCounterArray = [];
        //        var taskSystemDownArray = [];
        //        var taskManagerApprovalArray = [];
        ...
        for (var i = 0; i < taskscount; i++) {
            //     console.log('taskID: ' + allTasks[i].ID);
            console.log('data - TaskID: ' + data[i]['ID']);
            ...
            taskNotesString = '';
            taskCounterString = '';
            taskSystemDownString = '';
            taskManagerApprovalString = '';
            ...
            $http({
                method: "GET",
                url: "api/Tasks/" + TaskID, //$routeParams.taskId,
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                headers: fsaApp.session.getAuthenticationHeader()
            })
            .success(function (data, status, headers, config) {
                ...
                $scope.taskDetailsString = JSON.stringify(data);
                $scope.incidentID = data.TSK.IncidentID;
                incidentID = data.TSK.IncidentID;
                taskDetailsArray.push({ TaskID: TaskID, Details: $scope.taskDetailsString });              
                ...
                /*
                                $http({
                                    method: "GET",
                                    url: "api/Tasks/" + $scope.incidentID + "/GetCounter",     //$scope.tskD.TSK.IncidentID + "/GetCounter",
                                    contentType: "application/json; charset=utf-8",
                                    dataType: "json",
                                    headers: fsaApp.session.getAuthenticationHeader()
                                })
                                .success(function (data, status, headers, config) {               
                                    taskCounterString = JSON.stringify(data);
                                    //       console.log('taskNotesString: ' + taskNotesString);
                                    taskCounterArray.push({ TaskID: TaskID, Details: $scope.taskCounterString });
                                    var taskCounterArr = window.localStorage.getArray('taskCounter');
                                    for (var i = 0; i < taskCounterArr.length; i++) {
                                        var value = taskCounterArr[i];
                                        console.log('getting counter');
                                        var key = taskCounterArr[i]['TaskID'];
                                        var valuestr = taskCounterArr[i]['Details'];
                                        var tskCounter = JSON.parse(valuestr);
                                        console.log('tskCounter: ' + valuestr);
                                    }
                                })
                                .error(function (data, status, headers, config) {
                                ...
                                $http({
                                    method: "GET",
                                    url: "api/Tasks/" + TaskID + "/GetManagerApproval",        //$routeParams.taskId + "/GetManagerApproval",
                                    contentType: "application/json; charset=utf-8",
                                    dataType: "json",
                                    headers: fsaApp.session.getAuthenticationHeader()
                                })
                                .success(function (data, status, headers, config) {
                                    if (data != "\"null\"") {
                                        taskManagerApprovalString = JSON.stringify(data);
                                        var taskCounterArr = window.localStorage.getArray('taskManagerApproval');
                        //     console.log('taskManagerApprovalString: ' + taskManagerApprovalString);
                                        for (var i = 0; i < taskManagerApprovalArr.length; i++) {
                                            var value = taskManagerApprovalArr[i];
                                            console.log('getting ManagerApproval');
                                            var key = taskManagerApprovalArr[i]['TaskID'];
                                            var valuestr = taskManagerApprovalArr[i]['Details'];
                                            var tskManagerApproval = JSON.parse(valuestr);
                                            console.log('tskManagerApproval: ' + valuestr);
                                        }
                                    }
                                })
                                .error(function (data, status, headers, config) {
                ...
                */
                console.log('taskDetailsArray.length = ' + taskDetailsArray.length);
                console.log('data.length = ' + taskscount);
                if (taskDetailsArray.length == taskscount) {
                    window.localStorage.setArray("taskDetails", taskDetailsArray);
                    window.localStorage.setArray("taskNotes", taskNotesArray);
                    window.localStorage.setArray("taskCounter", taskCounterArray);
                    window.localStorage.setArray("taskManagerApproval", taskManagerApprovalArray);
                    ...
                    var taskDetsArr = window.localStorage.getArray('taskDetails');
                    for (var i = 0; i < taskDetsArr.length; i++) {
                        var value = taskDetsArr[i];
                     //   console.log('value: ' + value);                        
                        key = taskDetsArr[i]['TaskID'];
                        if (key == allTasks[i].ID) {
                            var valuestr = taskDetsArr[i]['Details'];
                            $scope.tskD = JSON.parse(valuestr);
                   //         console.log('tskDetails: ' + valuestr);
                            console.log('tskDetails.TSK.TaskNumber: ' + $scope.tskD.TSK.TaskNumber);
                            //====================
                       //     $scope.tskD = data;
        ...
        .error(function (data, status, headers, config) {
            ...
            console.log('data[i][-IncidentID-]: ' + data[i]['IncidentID']);

            $http({
                method: "GET",
                url: "api/Tasks/" + data[i]['IncidentID'] + "/GetTaskNotes",       //$scope.tskD.TSK.IncidentID + "/GetTaskNotes",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                headers: fsaApp.session.getAuthenticationHeader()
            })
            .success(function (data, status, headers, config) {
                ...
                taskNotesString = JSON.stringify(data);
                console.log('taskNotesString: ' + taskNotesString);
                taskNotesArray.push({ TaskID: TaskID, Details: $scope.taskNotesString });
                console.log('notes; taskNotesArray.length: ' + taskNotesArray.length);
                if (taskNotesArray.length == taskscount) {
                    window.localStorage.setArray("taskNotes", taskNotesArray);
                    console.log('loadAllNotes');
                    var taskNotesArr = window.localStorage.getArray('taskNotes');
                    console.log('taskNotesArr.length: ' + taskNotesArr.length);

                    for (var n = 0; n < taskNotesArr.length; n++) {
                        var value = taskNotesArr[n];
                        console.log('getting notes; index: ' + n + '; taskid: ' + taskNotesArr[n]['TaskID'] + '; details: ' + taskNotesArr[n]['Details']);
                        var key = taskNotesArr[n]['TaskID'];
                        var valuestr = taskNotesArr[n]['Details'];
                        console.log('valuestr: ' + valuestr);
                        if (valuestr != undefined) {
                            var tskNotes = JSON.parse(valuestr);
                        } else {
                            var tskNotes = '';
                        }
                        console.log('tskNotes: ' + valuestr);
                    }
                }
            })
        .error(function (data, status, headers, config) {
        ...
        //          var temparray = [{ 'TaskID': TaskID, Details: $scope.taskDetailsString }, { TaskID: TaskID, Details: $scope.taskDetailsString }, { TaskID: TaskID, Details: $scope.taskDetailsString }];
        //          temparray[0]['TaskID'];
       
        //      taskDetailsArray.push({ TaskID: allTasks[i].ID, Details: taskDetailsString });
        //            taskNotesArray.push({ TaskID: allTasks[i].ID, Notes: taskNotesString });
        //            taskCounterArray.push({ TaskID: allTasks[i].ID, Details: taskCounterString });
        //            taskSystemDownArray.push({ TaskID: allTasks[i].ID, Details: taskSystemDownString });
        //            taskManagerApprovalArray.push({ TaskID: allTasks[i].ID, Details: taskManagerApprovalString });
        //        }
        //       console.log('out of loop: ' + $scope.taskDetailsArray);
     
        /*      window.localStorage.setArray("taskDetails", taskDetailsArray);
              window.localStorage.setArray('taskNotes', taskNotesArray);
              window.localStorage.setItem('taskCounter', taskCounterArray);
              window.localStorage.setItem('taskSystemDown', taskSystemDownArray);
              window.localStorage.setItem('taskMangerApproval', taskManagerApprovalArray);
      
              console.log('taskDetailsArray: ' + taskDetailsArray);
              var thevalue = window.localStorage.getArray('taskDetails');
              
              console.log('value string: ' + thevalue + '; length = ' + thevalue.length);
             */
        //      for (var i = 0; i < thevalue.length; i++) {
        //          var value1 = thevalue[i];

        //               console.log('value1: ' + value1);
         
        //         tskDetails: "json"

        //        var tskDetails = JSON.parse(value1);
        //       value1.getItem('TaskID: 744881');
        //      console.log('tskNotes: ' + tskNotes);
        //    }
  
        // var value1 = JSON.parse(localStorage.getItem('744882'));
        // console.log('value json: ' + value1);
    ...
    .success(function (data, status, headers, config) {
        ...
        .success(function (data, status, headers, config) {
            if (data.srType.toString == "11002") {
                if (data.sysDown != "null" && (data.sysDown == "1" || data.sysDown == "Yes")) {        //confirm down should be required only when system is DOWN and SR type is General Repair
